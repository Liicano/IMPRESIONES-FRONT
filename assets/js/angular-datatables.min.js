...
(function (dt) {
    ...
    var TableController = (function () {
        function TableController($parse, $scope, $element, $attrs, $transclude, $rootScope, $q, $http, $compile, $templateCache, $injector) {
            ...
            // Complementos oyentes de eventos. $ emitir no es una opción ya que los eventos se transmitirán a través de diferentes tablas
            this.eventListeners = {
                'cellCompile': [],
                'cellPostLink': [],
                'cellPreLink': [],
            ...
            esto. $ attrs = $ attrs;
            esto. $ rootScope = $ rootScope;
            esto. $ compile = $ compile;
            esto. $ inyector = $ inyector;
            this. $ templateCache = $ templateCache;
        }
        // Ejecutará el selector desde la raíz del nodo pasado como parámetro
        TableController.executeSelector = function (selector, nodo) {
            var parent = null;
            var currentNode = nodo;
            while (currentNode) {
                parent = currentNode;
                currentNode = currentNode.parentNode;
            }
            if (instancia principal de DocumentFragment)
                parent = parent.children;
            var elem = $ (selector, principal);
            if (! elem.length)
                elem = $ (selector);
            regreso elem;
        };
        ...
        TableController.prototype.setupTable = function () {
            if ($ ('tfoot> tr> th', this. $ element) .length)
                this.declarativeFooter = verdadero;
        ...
        TableController.prototype.loadTemplates = function (onSuccess) {
            var _this = esto;
            // carga las plantillas faltantes
            if (this.templatesToLoad.length) {
                ...
                this. $ q.all (promesas) .then (función () {
        ...
        TableController.prototype.initializePlugins = function (dtSettings) {
            ...
            angular.forEach(dtSettings.aoColumns, function (oCol, idx) {
                ...
                angular.forEach (col, function (val, key) {
                    if (oCol.hasOwnProperty (clave) || val === undefined)
                        regreso;
                    oCol [clave] = val;
                });
            });
        ...
        TableController.prototype.destroyActivePlugin = function (plugin) {
            var _this = esto;
            // eliminar los oyentes de eventos
        ...
        TableController.prototype.initialize = function () {
            var _this = esto;
            this.templatesToLoad.length = 0; //Reiniciar
        ...
        TableController.prototype.onCollectionChange = function (newValue) {
            ...
            if (removed.length) {
                if (depuración)
                    console.time ('Ejecutando filas, devoluciones de llamada eliminadas. Elementos eliminados:' + removed.length);
                angular.forEach (settings.rowsRemoved, function (fn) {
                    if (angular.isFunction (fn))
                        fn.call (_this, eliminado);
                });
                if (depuración)
            ...
            if (added.length) {
                ...
                angular.forEach (settings.rowsAdded, function (fn) {
                    if (angular.isFunction (fn))
                        fn.call (_this, agregado);
                });
                if (depuración)
                    console.timeEnd ('Executing rowsAdded callbacks. Elementos agregados:' + added.length);
            }
            ...
            if (removed.length || added.length || rowsReordered) {
                if (added.length)
                    api.gotoLastPage (); // Solo necesitamos cambiar la página cuando se agrega un nuevo elemento y se mostrará en una nueva página
                api.draw (falso);
            }
            if (depuración)
